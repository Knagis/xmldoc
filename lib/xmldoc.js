...
XmlElement.prototype.toStringWithIndent = function(indent, options) {
  ...
  var linebreak = options && options.compressed ? "" : (options ? options.newline || "\n": "\n");
  
  for (var name in this.attr)
    if (Object.prototype.hasOwnProperty.call(this.attr, name))
        s += " " + name + '="' + this.attr[name] + '"';

  var finalVal = this.val === null ? null : this.val.replace(/</g, "&lt;").replace(/>/g, "&gt;");

  if (finalVal !== null && options && options.trimmed && finalVal.length > 25)
    finalVal = finalVal.trim().substring(0,25).trim() + "…";
  ...
  if (this.children.length) {
    ...
    if (finalVal !== null && (finalVal.length || !this.children.length))
